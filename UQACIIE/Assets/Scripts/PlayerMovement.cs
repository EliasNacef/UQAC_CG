...
public class PlayerMovement : MonoBehaviour {
    ...
    public float mapXMin; // Limite min en X ou le joueur peut aller
    public float mapYMin; // Limite min en Y ou le joueur peut aller
    public float mapXMax; // Limite max en X ou le joueur peut aller
    public float mapYMax; // Limite max en Y ou le joueur peut aller
    ...
    public IEnumerator ChangeCell()
    {
        while(true) // Boucle infinie
        {
            ...
            if (move.x != 0) // Si on souhaite se déplacer selon X
            {
                ...
                if (currentPosition.x + movingX < mapXMax && currentPosition.x + movingX > mapXMin) transform.position = new Vector3(currentPosition.x + movingX, currentPosition.y, currentPosition.z); // Déplacement sur la future cellule
                movingX = 0;
            ...
            else if (move.y != 0) // Sinon si on souhaite se deplacer selon Y
            {
                ...
                if (currentPosition.y + movingY < mapYMax && currentPosition.y + movingY > mapYMin) transform.position = new Vector3(currentPosition.x, currentPosition.y + movingY, currentPosition.z); // Déplacement sur la future cellule
                movingY = 0;
                yield return new WaitForSeconds(0.15f);
            }
